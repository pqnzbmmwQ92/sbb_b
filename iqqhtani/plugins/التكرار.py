...
@iqqhtani.iq_cmd(pattern="تكرار حزمه الملصقات$",)
...
@iqqhtani.iq_cmd(    pattern="تكرار_احرف ([\s\S]*)",)
...
@iqqhtani.iq_cmd(    pattern="تكرار_كلمه ([\s\S]*)",)
async def tmeme(event):
...
@iqqhtani.iq_cmd(pattern=f"{MUQT} ([\s\S]*)",)
async def spammer(event):
    ...
    try:
        ...
        return await edit_delete(            event, "عذرا طريقه كتابة الأمر خطأ - شرح الارسال الوقتي او مؤقت للقروبات هنا : https://t.me/EITH_1"        )
    cat = input_str[1:]
    try:
        int(cat[0])
    except Exception:
...
@iqqhtani.iq_cmd(pattern=f"{UNMUQT} ?(.*)",)
async def spammer(event):
    reply = await event.get_reply_message()
    await event.delete()
    delgvar("spamwork")

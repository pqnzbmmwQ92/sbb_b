...
IqqhtaniPC = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/1b62f55aaf2c8b5473a02.jpg"
...
IQPIC = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/1b62f55aaf2c8b5473a02.jpg"
...
async def iq(iqqhtanievent):
    ...
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "اهلا وسهلا بك عزيزي في سورس  Ξ𝗜𝗧𝗛𝗢𝗡™ 𓃠"
    Iqqhtani_IMG = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/1b62f55aaf2c8b5473a02.jpg"
...
fahs = """✪ : انا  {my_mention}  𓇡.
✪ : الوقت  {TM}  𓇡.
✪ : وقت التشغيل  {uptime}  𓇡.
✪ : بوتي  {tg_bot}  𓇡.
✪ : البنك  {ping}  𓇡.
✪ : الاصدار 1.1.1  𓇡.
✪ : سورس Ξ𝗜𝗧𝗛𝗢𝗡™ : @EITHON1  𓇡."""
@iqqhtani.on(admin_cmd(pattern="رابط التنصيب(?: |$)(.*)"))
async def source(e):
    await edit_or_reply(e, "https://github.com/pqnzbmmwQ92/zuhairy",)
...
async def amireallyalive(event):
    ...
    Iqqhtani_ALIVE_TEXT = "❬ سورس ايــثــون - Ξ𝗜𝗧𝗛𝗢𝗡™ ، 🕸  ❭ :"
    Iqqhtani_IMG = gvarstatus("ALIVE_PIC")
    if Iqqhtani_IMG:
        CAT = [x for x in Iqqhtani_IMG.split()]
        A_IMG = list(CAT)
        PIC = random.choice(A_IMG)
        cat_caption += f"**❬ ٰوقت الـتشغيل  : {uptime}  ٍَ❭**"
        try:
            await event.client.send_file(event.chat_id, PIC, caption=cat_caption, reply_to=reply_to_id)
            await event.delete()
        except (WebpageMediaEmptyError, MediaEmptyError, WebpageCurlFailedError):
            return await edit_or_reply(event, f"**وقت التشغيل")
    else:
        await edit_or_reply(event, f"**❬ ٰوقت الـتشغيل  : {uptime}  ٍَ❭**")
@iqqhtani.on(admin_cmd(pattern="فارات تنصيبي(?: |$)(.*)"))
async def _(event):
    cmd = "env"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = (f"✪︙  وحـدة المعلومات الخاصه بتنصيبك مع جميع الفارات  لتنصيب سورس ايــثــون @EITHON1 :**\n\n{o}")
...
async def _(event):
    ...
    await asyncio.sleep(1)
    await edit_or_reply(event , "حسنا جاري تحديث ايــثــون انتضر من 5 الى 10 دقائق")
    try:
...
قناة الكلايش  : @EITH_0
قناه شروحات الاوامر  : @EITH_1
قناه المتغيرات او الفارات : @EIT_2""")
    ...
    if vname in vlist:
        ...
        if cmd == "اضف":
            if not vinfo and vname == "ALIVE_TEMPLATE":
                return await edit_delete(event, f" ✪︙  📑 يرجى متابع قناه الفارات تجدها هنا : @EITHON1")
    ...
    if exe == "get":
        ...
        try:
            ...
            if variable in heroku_var:
                return await ics.edit(                    "𓆩 𝑺𝑶𝑼𝑹𝑪𝑬  - Ξ𝗜𝗧𝗛𝗢𝗡™ 𝑽𝑨𝑹𝑺 𓆪\n𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻"                    f"\n **⌔** `{variable} = {heroku_var[variable]}` .\n"                )
            return await ics.edit(                "𓆩 𝑺𝑶𝑼𝑹𝑪𝑬 - Ξ𝗜𝗧𝗛𝗢𝗡™ 𝑽𝑨𝑹𝑺 𓆪\n𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻"                f"\n **⌔ خطا :**\n-> {variable} غيـر موجود. "            )
        except IndexError:
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inlineiqqhtani(iqqhtani):
        ...
        if query.startswith("تنصيب") and iqqhtani.query.user_id == bot.uid:
            buttons = [[Button.url("1- شرح التنصيب", "https://youtu.be"), Button.url("2- استخراج ايبيات", "https://my.telegram.org/"),],[Button.url("3- ستخراج تيرمكس", "https://replit.com/@telethon-Arab/generatestringsession#start.sh"), Button.url("4- بوت فاذر", "http://t.me/BotFather"),],[Button.url("5- رابط التنصيب", "https://dashboard.heroku.com/new?template=https://github.com/pqnzbmmwQ92/zuhairy"),],[Button.url("المطـور 👨🏼‍💻", "https://t.me/EITHON1"),]]
...
( .ايقاف اسم وقتي )
———————×———————
 ✪︙ يوجد شرح مفصل عن الامر هنا : https://t.me/EIT_2
""")
...
الشرح :   يضيفلك جميع الاعضاء الي برابط القروب يضيفهم بقروبك
 ———————×———————
يوجد شرح بتفصيل هنا : https://t.me/EITHON1
شرح الأوامر : ( @EITH_1 ) .
قناه السورس : ( @EITHON1 ) .
جميع الاوامر تكون بدايتها نقطة .
""")
...
———————×———————
يوجد شرح مفصل للتكرار هنا : https://t.me/EITHON1
قناه السورس : ( @EITHON1 ) .
جميع الاوامر تكون بدايتها نقطة .
""")
...
✐ :  يعطيك رموز للزخرفه2
———————×———————
يوجد شرح مفصل عن اوامر زخرفه هنا : https://t.me/EITHON1
""")
...
———————×———————
  ⦑   شرح اوامر الوسائط هنا :  https://t.me/EITHON1  ⦒
""")
...
———————×———————
  ⦑   شرح اوامر الملصقات هنا  :  https://t.me/EITHON1  ⦒
""")
...
———————×———————
الأمر ⦙ ( .المده )
الشرح  : يضهر لك مدة تشغيل بوت ايــثــون لديك
———————×———————
الأمر ⦙ ( .تحميل ملف + الرد ع الملف )
الشرح : يحمل ملفات ايــثــون
———————×———————
الأمر ⦙ ( .مسح ملف + الرد ع الملف )
الشرح :  يمسح الملف الي حملته
———————×———————
الأمر ⦙ ( .تحديث )
الشرح :  امر لأعاده التشغيل وتحديث ملفات السورس وتسريع ايــثــون
———————×———————
الأمر ⦙ ( .اطفاء مؤقت + عدد الثواني )
الشرح : يقوم بأطفاء ايــثــون بعدد الثواني الي ضفتها  عندما تخلص الثواني سيتم اعاده تشغيل ايــثــون
———————×———————
الأمر ⦙ ( .الاوامر )
...
———————×———————
الأمر ⦙ ( .استخدامي )
الشرح :   يضهر لك كمية استخدامك ايــثــون
———————×———————
الأمر ⦙ ( .تاريخ التنصيب )
...
الشرح : اي شخص يعملك تاك او يراسلك او يرد عليك يرد عليه ايــثــون بكليشة انا حاليا غير موجود ويضع له السبب الي نتة وضعته
———×———

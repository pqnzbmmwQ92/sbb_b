...
@iqqhtani.tgbot.on(CallbackQuery(data=re.compile(r"^age_verification_true")))
...
@iqqhtani.tgbot.on(CallbackQuery(data=re.compile(r"^age_verification_false")))
...
@iqqhtani.tgbot.on(CallbackQuery(data=re.compile(r"^chg_of_decision_")))
async def chg_of_decision_(event: CallbackQuery):
    ...
    try:
        await event.edit(
            ...
            file="https://telegra.ph/file/2ed67fafed303714aabe4.jpg",
            buttons=buttons,
        )
    except MessageNotModifiedError:
        pass
